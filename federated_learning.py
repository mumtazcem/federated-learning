    ...
    for xp_count, xp in enumerate(experiments):
        ...
        for c_round in range(1, hp['communication_rounds'] + 1):
            ...
            for client in participating_clients:
                xp.log({'client{}_synchronize_with_server at '.format(client.id): time.time() - t1}, printout=True)
                client.synchronize_with_server(server)
                xp.log({'client{}_compute_weight_update at '.format(client.id): time.time() - t1}, printout=True)
            ...
            # Server does
            xp.log({'aggregate_weight_updates at ': time.time() - t1}, printout=True)
